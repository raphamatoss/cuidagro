...
const symptomGroups = [
  {
    ...
    items: [
      { id: 'skin_irritation', label: 'Irritação / Vermelhidão', description: 'Pele quente, dolorosa ou inchada' },
      { id: 'skin_dry', label: 'Desidratação / Secura', description: 'Pele descascando ou esbranquiçada' },
      { id: 'skin_allergy', label: 'Alergia / Coceira', description: 'Brotoejas ou manchas' },
    ]
  },
  {
    category: "Respiração",
    ...
    items: [
      { id: 'resp_nose', label: 'Ardência no nariz/boca', description: '' },
      { id: 'resp_cough', label: 'Tosse seca ou cheia', description: '' },
      { id: 'resp_chest', label: 'Dor no peito', description: 'Dificuldade para respirar fundo' },
    ]
  },
  {
  ...
  },
  {
    ...
    items: [
      ...
      { id: 'gen_weakness', label: 'Fraqueza / Cansaço', description: '' },
      { id: 'gen_cramps', label: 'Câimbras ou Tremores', description: '' },
    ]
  }
];
...
export default function SymptomsPage() {
  ...
  const handleSubmit = async () => {
    if (selectedSymptoms.length === 0) {
      alert("Por favor, selecione pelo menos um sintoma ou volte para a home.");
      return;
    }
    ...
    <div className="flex min-h-screen flex-col bg-gray-50 pb-28">
      ...
      <header className="bg-white px-4 py-4 shadow-sm sticky top-0 z-20 flex items-center gap-4">
        ...
        <div>
          ...
          <h1 className="text-[1rem] font-bold text-gray-800">O que você está sentindo agora?</h1>
        </div>
      </header>
      ...
      <main className="p-4 flex flex-col gap-6">
        {symptomGroups.map((group) => (
          <section key={group.category} className="bg-white rounded-xl pb-4 shadow-sm border border-gray-100">
            
            {/* Título da Categoria*/}
            <h2 className={`font-bold text-sm uppercase tracking-wide mb-4 p-4 rounded-t-lg w-full ${group.color}`}>
              {group.category}
            </h2>
            ...
            <div className="flex flex-col gap-3 px-3">
              {group.items.map((item) => {
                ...
                return (
                  ...
                  >
                    {/* Ícone de Checkbox */}
                    <div className={`mt-0.5 transition-colors ${isSelected ? 'text-agro-blue' : 'text-gray-300'}`}>
                      {isSelected ? <CheckCircle2 size={24} fill="var(--color-agro-blue-light)" /> : <Circle size={24} />}
                    </div>

                    <div>
                      <span className={`block font-semibold ${isSelected ? 'text-agro-blue' : 'text-gray-700'} text-smg`}>
                        {item.label}
                      </span>
                      {item.description && (
